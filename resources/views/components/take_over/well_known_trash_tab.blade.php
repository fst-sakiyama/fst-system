<table class="table mb-0 table-hover">
	...
	@foreach($wellKnownsTrash as $item)
		<tr class="d-flex" id = "{{'item_'.$item->takeOverId}}">
			...
			<td class="col-2"><a class="trashWellKnownTable" data-class = "{{'item_'.$item->takeOverId}}" style="cursor:pointer;">{{ mb_substr($item->project->project->projectName,0,12) }}</a></td>
			...
			<td class="col-2"><a class="trashWellKnownTable" data-class = "{{'item_'.$item->takeOverId}}" style="cursor:pointer;"><span class="small">{{ mb_substr($item->project->project->client->clientName,0,8) }}</span></td>
		</tr>
	@endforeach
	</tbody>
</table>
